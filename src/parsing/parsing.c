...
/*   Updated: 2025/10/15 18:23:05 by xueyang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_scene(char **lines, t_cub_data *d)
{
	...
	if (!lines || !d)
		return (printf("Error: invalid parse input"), 0);
	i = 0;
	while (lines[i] && !is_map_line(lines[i]))
	{
		r = parse_header_line(d, lines[i]);
		if (r == -1)
			return (printf("Error: parse header"), 0);
		if (r == 0 && is_nonblank_line(lines[i]))
			return (printf("Error: unexpected blank line"), 0);
		i++;
	}
	if (!find_map_range(lines, &s, &e))
		return (printf("Error: map block not found"), 0);
	d->map = create_game_map(lines, s, e);
	if (!d->map)
		return (printf("Error: map not created"), 0);
...
int	load_and_parse(char *path, t_cub_data *d)
{
	...
	if (!path || !d)
		return (printf("Error: invalid input"), 0);
	if (!check_file(path))
		return (printf("Error: cub file"), 0);
	if (!load_cub_file(path, &lines))
		return (printf("Error: file reading"), 0);
